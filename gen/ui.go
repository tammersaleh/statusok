	...
	for i, a := range blog.Articles {
		...
		if len(authors) == 1 {
			item.Author = &jsonfeed.Author{
				Name: authors[0].Name,
				URL:  blog.URL + "/authors/" + authors[0].ID,
			}
		}
        ...
        <div class="lede__byline">
          {{- range .Authors }}
            <div class="lede__author">
              <a href="authors/{{.ID}}" rel="author">{{.Name}}</a>
            </div>
          {{- end }}
...
var authorPage = template.Must(template.New("author").Parse(`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8" />
  <meta http-equiv="x-ua-compatible" content="ie=edge">
